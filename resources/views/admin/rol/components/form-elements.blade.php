...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('permission'), 'has-success': fields.permission && fields.permission.valid }">
    <label for="permission" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-3'">{{ trans('admin.user.columns.permission') }}</label>
    <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-7'">
        <multiselect v-model="form.permission" placeholder="{{ trans('brackets/admin-ui::admin.forms.select_options') }}" label="name" track-by="id" :options="{{ $permission->toJson() }}" :multiple="true" open-direction="bottom"></multiselect>
